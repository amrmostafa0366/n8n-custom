...
export class Aws implements ICredentialType {
	...
	async authenticate(
		...
		const requestWithUri = requestOptions as unknown as IHttpRequestOptions;
		console.log('requestWithUri');
		console.log(requestWithUri);
		if (requestWithUri.url) {
			requestOptions.url = requestWithUri.url;
		...
		try {
		const signature =	sign(signOpts, securityHeaders);
	...
	test: ICredentialTestRequest = {
		request: {
			baseURL: '=http://oss.eu-west-1.prod-cloud-ocb.orange-business.com/',
